        ...
        if len(user_result) == 1:
            ...
            if user_password == user_password_:
                ...
                if len(user_r) == 0 :#如果在医生表里没有找到相应的记录
                    return render(request, 'User/user_choose.html', context=context)#跳转到管理员界面
                else:
                    context["info"] = "身份错误！！！"
                    context["status"] = 3
                    HttpResponse(request, 'User/new_login.html', context=context)
            else:
                context["info"] = "密码错误！！！"
                context["status"] = 1
...
def user_choose(request):
    return render(request, 'User/user_choose.html')
...
def search_by_patient_name(request):
        ...
        info_dic = {}
        if d is None:
            return render(request, 'Patient/search_by_patient_name.html', context=info_dic)
